...
class DataEngineeringFlow(Flow[DataEngineeringState]):
    ...
    def _track_crew_usage(self, crew: Crew) -> None:
        for agent in crew.agents:
            role = agent.role
            if not hasattr(agent, "llm") or not hasattr(agent.llm, "get_token_usage_summary"):
                continue
            try:
                usage  = agent.llm.get_token_usage_summary()
                bucket = self.state.agent_token_usage.setdefault(
                    role,
                    {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0, "successful_requests": 0},
                )
    ...
    def _entity_map_text(self) -> str:
        return "\n".join(f"  - {fn}: {entity}" for fn, entity in self.state.entity_map.items())
    ...
    def profile_datasets(self) -> None:
        ...
        for report in [
            "profiling_report.json", "quality_report.md", "schema_design.md",
            "transformations.sql", "kpi_report.md", "executive_summary.md",
        ]:
        ...
        discovered = [
            f for f in os.listdir(self.state.data_dir)
        ...
        for filename, raw_profile in combined_results.items():
            ...
            classification = EntityClassifier.classify(cols, row_count=row_count, filename=filename)
    ...
    def check_quality_threshold(self) -> str:
        if self.state.quality_score < 60:
            if not sys.stdin.isatty():
                ...
                if not HumanLoopService.request_human_approval(self.state.quality_score, summary):
    ...
    def design_schema(self) -> None:
        ...
        task      = TaskFactory({"warehouse_architect": architect}).create_schema_design_task()
    ...
    def plan_transformations(self) -> None:
        ...
        try:
            self.state.source_row_counts = DatabaseService.count_source_rows(self.state.data_dir)
            print(f"[Flow] Source row counts: {dict(sorted(self.state.source_row_counts.items(), key=lambda x: -x[1]))}")
        except Exception as e:
            print(f"[Flow] Warning: Could not count source rows: {e}")

        planner       = SchemaPlanner(self.state.data_dir, self.state.star_schema, self.state.source_row_counts)
        ...
        plan_crew = Crew(
            agents=[architect],
            tasks=[TaskFactory({"warehouse_architect": architect}).create_schema_plan_task()],
            verbose=True,
        )
        plan_raw    = plan_crew.kickoff(inputs={
        ...
        if not self.state.primary_fact_table:
            raise ValueError("No Fact_ tables were successfully created. Pipeline cannot continue.")
        ...
        try:
            ...
            val_crew = Crew(
                agents=[ve],
                tasks=[TaskFactory({"validation_engineer": ve}).create_validation_task()],
                verbose=True,
            )
            val_res  = val_crew.kickoff(inputs={
            ...
            with open(os.path.join(self.state.reports_dir, "validation_report.md"), "w", encoding="utf-8") as f:
                f.write(val_res.raw)
        ...
        self.state.verified_metrics = compute_verified_metrics(self.state.db_path, self.state.primary_fact_table)
        print(f"[Flow] Verified metrics: {list(self.state.verified_metrics.get('fact_tables', {}).keys())}")
    ...
    def run_analytics(self) -> None:
        ...
        task      = TaskFactory({"analytics_engineer": analytics}).create_business_insights_task()
